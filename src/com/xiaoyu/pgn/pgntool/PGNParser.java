        ...
        for (String pair : pairs) {
            ...
            if (game.containsTagKey(FEN)) {
                ...
                if (rawMoves.length > 0) {
                    ...
                    String substring = rawMove.substring(rawMove.length() - 2);
                    int pos = Pos.fromString(substring);
                    if (pos <= 31) {
                        color[0] = BLACK;
                    } else {
                        color[0] = WHITE;
...
//			printBoard(board);
    ...
    public static byte[][] createFENBoard(String fen, int[] color) {
        byte[][] bytes = new byte[8][8];//初始化每个格子都是EMPTY
        ...
        while (pos < 64 && i < fen.length()) {
            ...
            if (s.equals("k")) {
                bytes[x][y] = BLACK_KING;
            } else if (s.equals("K")) {
                bytes[x][y] = WHITE_KING;
            } else if (s.equals("q")) {
                bytes[x][y] = BLACK_QUEEN;
            } else if (s.equals("Q")) {
                bytes[x][y] = WHITE_QUEEN;
            } else if (s.equals("r")) {
                bytes[x][y] = BLACK_ROOK;
            } else if (s.equals("R")) {
                bytes[x][y] = WHITE_ROOK;
            } else if (s.equals("b")) {
                bytes[x][y] = BLACK_BISHOP;
            } else if (s.equals("B")) {
                bytes[x][y] = WHITE_BISHOP;
            } else if (s.equals("n")) {
                bytes[x][y] = BLACK_KNIGHT;
            } else if (s.equals("N")) {
                bytes[x][y] = WHITE_KNIGHT;
            } else if (s.equals("p")) {
                bytes[x][y] = BLACK_PAWN;
            } else if (s.equals("P")) {
                bytes[x][y] = WHITE_PAWN;
            } else if (s.equals("/")) {
                iAdd = 0;
            } else {
        ...
        i++;
        if (i < fen.length()) {
            String[] arr = fen.substring(i).split(" ");
            if (arr.length > 0) {
                if (arr[0].equals("w")) {
                    color[0] = WHITE;
                } else {
